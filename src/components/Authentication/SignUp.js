...
const SignUp = ({handleClose}) => {
    ...
    const handleSubmit= async () =>{
        if(password!==confirmPassword)
            {
                setAlert({open:true,message:"Passowrds do not match",type:"error"});
                return;
            }
            try{
  ...
  return (
    <Box p={3} style={{display: "flex", flexDirection:"column",gap:"20px"}} >
        ...
        <Button variant='contained' size='large' style={{background:"#EEBC1D",color:"black"}} onClick={handleSubmit} >Sign Up</Button>
    </Box  >
  )
}
